...
const Menus = ({ siderFold, darkTheme, location, handleClickNavMenu, navOpenKeys, changeOpenKeys, menu }) => {
  ...
  const getMenus = (menuTreeN, siderFoldN) => {
    return menuTreeN.map(item => {
      ...
      let invisible = item.visible === false;
      if(! invisible) {
        if (item.children) {
          if (item.mpid) {
            levelMap[itemId] = item.mpid
          }
          return (
            <Menu.SubMenu
              key={itemId}
              title={<span>
              {item.icon ? <Icon type={item.icon}/> : <Icon type="appstore-o"/>}
                {(!siderFoldN || menuTree.indexOf(item) < 0) && (item.name || item.title)}
            </span>}
            >
